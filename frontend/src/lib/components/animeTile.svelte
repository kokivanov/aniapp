...
>
    <div class="w-48 h-64">
        ...
        >
            <div
                class="absolute bg-black bg-opacity-80 font-bold text-white opacity-0 hover:opacity-100 transition-opacity w-full h-full"
            >
                {#if isMarked}
                    <button on:click={toggleMark}>
                        <UnmarkIcon
                            class="absolute right-0 top-0 mr-2 mt-2 z-10 text-orange-500"
                        ></UnmarkIcon>
                    </button>
                {:else}
                    <button on:click={toggleMark}>
                        <MarkIcon class="absolute right-0 top-0 mr-2 mt-2 z-10"
                        ></MarkIcon>
                    </button>
                {/if}
                <a
                ...
                >
                    <p
                        class="h-12 w-5/6 text-ellipsis line-clamp-4 text-center"
                    >
                    ...
                    {#if anime.season && anime.season}
                        <p>
                            ...
                            {addNumberSuffix(anime.seasonYear + "")}
                        </p>
                    {:else}
            ...
            <div
                class="absolute font-sans font-semibold text-white text-xs rounded-r-md rounded-b-none bottom-0 left-0 {anime.status ==
                'FINISHED'
                    ? 'bg-green-500'
